...
st.info("💡 본 시스템은 [수도권정비계획법 시행령 별표1] 및 2026년 개정 세법을 준수합니다.")
...
# 1. 조특법 제6조 제3항 기준의 18개 업종 분류 테이블 생성
# 각 항목별로 (소득세감면여부, 취득세감면여부) 플래그를 정밀 매핑
    #BUSINESS_TAX_MAP = {
    #"선택하세요": {"inc": False, "tax": False},
    #"광업": {"inc": True, "tax": True},
    #"제조업 (유사 사업 포함)": {"inc": True, "tax": True},
    #"수도, 하수 및 폐기물 처리, 원료 재생업": {"inc": True, "tax": True},
    #"건설업": {"inc": True, "tax": True},
    #"통신판매업 (온라인 쇼핑몰, 스마트스토어 등)": {"inc": True, "tax": False},  # 취득세 제외
    #"물류산업": {"inc": True, "tax": True},
    #"음식점업 (식당, 카페, 제과점 등)": {"inc": True, "tax": False},  # 취득세 제외
    #"정보통신업 (비디오물감상실, 뉴스제공, 가상자산 제외)": {"inc": True, "tax": True},
    #"🚫 비디오물감상실, 뉴스제공, 가상자산 관련 업종": {"inc": False, "tax": False},
    #"금융 및 보험업 (정보통신 활용 금융서비스 한정)": {"inc": True, "tax": False},  # 취득세 제외
    #"전문, 과학 및 기술 서비스업 (연구개발업, 광고업, 기타과학기술, 전문디자인, 시장조사/여론조사)": {"inc": True, "tax": True},
    #"전문, 과학 및 기술 서비스업 (위 5개 분야 외 일반 전문/과학기술 서비스업)": {"inc": True, "tax": False},  # 취득세 제외
    #"🚫전문직(변호사, 변리사, 법무사, 회계사, 세무사, 수의사, 행정사, 건축사)": {"inc": False, "tax": False},
    #"사업시설 관리 및 조경 서비스업": {"inc": True, "tax": True},
    #"사업 지원 서비스업 (고용알선, 인력공급, 경비·경호, 보안시스템, 전시·컨벤션·행사대행)": {"inc": True, "tax": True},
    #"사업 지원 및 임대 서비스업 (위 열거된 분야 외 일반 사업지원/임대 서비스업)": {"inc": True, "tax": False},  # 취득세 제외
    #"사회복지 서비스업": {"inc": True, "tax": False},  # 취득세 제외
    #"창작 및 예술관련 서비스업 (자영예술가 제외)": {"inc": True, "tax": True},
    #"스포츠 및 기타 여가관련 서비스업 (오락장, 수상오락, 사행시설 제외 스포츠/여가업)": {"inc": True, "tax": False},  # 취득세 제외
    #"🚫 자영예술가, 오락장 운영업, 수상오락 서비스업, 사행시설 등": {"inc": False, "tax": False},
    #"개인 및 소비용품 수리업": {"inc": True, "tax": False},  # 취득세 제외
    #"이용 및 미용업": {"inc": True, "tax": False},  # 취득세 제외
    #"직업기술 분야 학원 또는 직업능력개발훈련시설 (훈련 주된 사업 한정)": {"inc": True, "tax": True},
    #"관광숙박업, 국제회의업, 테마파크업 및 관광객 이용시설업": {"inc": True, "tax": True},
    #"노인복지시설 운영 사업": {"inc": True, "tax": False},  # 취득세 제외
    #"전시산업": {"inc": True, "tax": True},
    #"🚫기타 법령 미명시 업종 (오프라인 도·소매업, 일반 보습/외국어학원, 부동산 임대/매매업 등)": {"inc": False, "tax": False}}
    ...
    # -------------------------------------------------------------------------
    # ZONE 1. 3대 세법 올패스 (소득세 감면 O / 취득세 감면 O / 부가세 일반과세 강제)
    # -------------------------------------------------------------------------
    "광업": {"inc": True, "tax": True, "vat": "일반"},
    "제조업 (유사 사업 포함)": {"inc": True, "tax": True, "vat": "일반"},
    "수도, 하수 및 폐기물 처리, 원료 재생업": {"inc": True, "tax": True, "vat": "일반"},
    "건설업": {"inc": True, "tax": True, "vat": "일반"},
    
    # -------------------------------------------------------------------------
    # ZONE 2. 실물 인프라/기술 창업 (소득세 감면 O / 취득세 감면 O / 부가세 간이과세 가능)
    # -------------------------------------------------------------------------
    "물류산업": {"inc": True, "tax": True, "vat": "간이"},
    "정보통신업 (비디오물감상실, 뉴스제공, 가상자산 제외)": {"inc": True, "tax": True, "vat": "간이"}, # SW개발, 출판 등
    "전문, 과학 및 기술 서비스업 (연구개발업, 광고업, 기타과학기술, 전문디자인, 시장조사/여론조사)": {"inc": True, "tax": True, "vat": "일반"}, # 엔지니어링 및 R&D 중심

    # -------------------------------------------------------------------------
    # ZONE 3. 국세(소득세)만 감면되고 지방세(취득세)는 배제되는 서비스/유통 구간
    # -------------------------------------------------------------------------
    "전문, 과학 및 기술 서비스업 (위 분야 외 일반 전문/과학기술)": {"inc": True, "tax": False, "vat": "일반"},
    "통신판매업 (온라인 쇼핑몰, 스마트스토어 등)": {"inc": True, "tax": False, "vat": "간이"},
    "음식점업 (식당, 카페, 제과점 등)": {"inc": True, "tax": False, "vat": "간이"},
    "금융 및 보험업 (정보통신 활용 핀테크·금융플랫폼 서비스 한정)": {"inc": True, "tax": False, "vat": "일반"}, # 핀테크는 부가세 '일반과세' 용역임
    "사업시설 관리 및 조경 서비스업": {"inc": True, "tax": False, "vat": "일반"},
    "사업 지원 서비스업 (고용알선, 인력공급, 경비·경호, 보안시스템, 전시·컨벤션·행사대행)": {"inc": True, "tax": False, "vat": "일반"},
    "사업 지원 및 임대 서비스업 (위 열거된 분야 외 일반 사업지원/임대)": {"inc": True, "tax": False, "vat": "일반"},
    "창작 및 예술관련 서비스업 (자영예술가 제외)": {"inc": True, "tax": False, "vat": "간이"},
    "스포츠 및 기타 여가관련 서비스업 (오락장, 수상오락, 사행시설 제외 스포츠/여가)": {"inc": True, "tax": False, "vat": "간이"},
    "개인 및 소비용품 수리업": {"inc": True, "tax": False, "vat": "간이"},
    "이용 및 미용업": {"inc": True, "tax": False, "vat": "간이"},
    "관광숙박업, 국제회의업, 테마파크업 및 관광객 이용시설업": {"inc": True, "tax": False, "vat": "간이"},
    "전시산업": {"inc": True, "tax": False, "vat": "일반"},
    "사회복지 서비스업": {"inc": True, "tax": False, "vat": "면세"}, 
    "직업기술 분야 학원 또는 직업능력개발훈련시설 (훈련 주된 사업 한정)": {"inc": True, "tax": False, "vat": "면세"}, 
    "노인복지시설 운영 사업": {"inc": True, "tax": False, "vat": "면세"}, 

    # -------------------------------------------------------------------------
    # ZONE 4. 🚫 창업 감면 전면 비대상 (조특법X, 지특법X) -> 부가세 성격별로 최종 분리
    # -------------------------------------------------------------------------
    "🚫 법률/회계/세무 전문직, 오락장, 가상자산거래소, 오프라인 도매업, 상가임대업 등": {"inc": False, "tax": False, "vat": "일반"},
    "🚫 기타 일반 오프라인 소매업, 옷가게, 잡화점 등": {"inc": False, "tax": False, "vat": "간이"},
    "🚫 전통 금융·보험업, 대부업, 보험대리점(GA), 주택임대, 일반 보습학원 등": {"inc": False, "tax": False, "vat": "면세"},
    "🚫 의원·치과·한의원, 약국, 동물병원 등": {"inc": False, "tax": False, "vat": "겸영"},}
...
#세법상 권역 분류 명칭 정밀화
    ...
    with res_col2:
        ...
        #else:
            #st.warning("🔶 **[과세유형] 연 매출 1.4억 원 이상: 일반과세자 대상 (간이과세 배제)**")
            
       # if "해외 수출" in is_export:
            #st.success("✈️ **[부가세법 제24조] 외화 획득 사업자 영세율(0%) 적용 대상**")
            
        # ⭐️ 부가가치세 조기환급 조건부 렌더링 코드로 수정완료 
            ...
            #if int(expected_sales) < 140000000:
                # ❌ 간이과세자는 환급 불가능하다는 세법 팩트를 정확히 명시
                #st.error("❌ **[환급 불가] 현재 '간이과세자'로 설정되어 있어 부가가치세 환급 및 조기환급이 불가능합니다.**")
                #st.caption("💡 [세무 팁] 인테리어 비용에 대한 부가세 10%를 환급받고 싶다면, 사업자등록 시 초기 매출이 적더라도 '일반과세자'를 의도적으로 선택하여 등록해야 합니다.")
            #else:
                # ⭕ 일반과세자일 때만 환급액 계산 및 출력
