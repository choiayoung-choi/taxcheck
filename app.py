...
with col1:
   ...
   # [대한민국 세법 고증 100%] 조특법(inc) - 지특법(tax) - 부가세(vat) 최종 마스터 테이블
    ...
    # -------------------------------------------------------------------------
    # ➕ ZONE 5. 면세 및 겸영 독립 트랙 구간 (약국 및 병의원 세법 고증 반영)
    # -------------------------------------------------------------------------
    "일반 중고등학교 교과 학원 및 입시학원 (부가세 면세 대상 법정 학원)": {"inc": True, "tax": False, "vat": "면세"}, 
    "🚫 일반 병의원, 한의원, 치과의원 (의료 보건 용역)": {"inc": False, "tax": False, "vat": "면세"}, 
    "농수산물 오프라인 도소매 및 유통 사업 (부가세 면세 물품 취급)": {"inc": False, "tax": False, "vat": "면세"}, 
    "🚫 약국 (처방조제=면세 + 일반의약품 판매=과세)": {"inc": False, "tax": False, "vat": "겸영"}}
...
# [STEP 3] 부가가치세 조기환급 조건부 렌더링 (면세, 겸영, 간이, 일반 완벽 분기)
        ...
        if property_plan == "있음" and property_price > 0:
          ...
          if is_eligible_property_tax:
             ...
             if location_status != "내":
                 ...
                 st.error(f"🚨 **[지특법 제58조의3 제7항 사후관리] 창업일로부터 4년 내 취득 후, 3년 이상 해당 사업에 직접 사용하지 않고 매각/타 용도 전용 시 추징 리스크 금액: {int(potential_penalty):,}원**")
             else:
        ...
        st.subheader("5. 창업자금 증여세 과세특례(조세특례제한법)")
